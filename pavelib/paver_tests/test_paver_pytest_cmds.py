...
class TestPaverPytestCmd(unittest.TestCase):
    ...
    def test_SystemTestSuite_suites(self, system):
        ...
        self.assertEqual(suite.cmd, self._expected_command(system, test_id, "SystemTestSuite"))

    @ddt.data('lms', 'cms')
    def test_SystemTestSuite_auto_processes(self, system):
        test_id = 'tests'
        suite = SystemTestSuite(system, test_id=test_id, processes=-1)
        self.assertEqual(suite.cmd, self._expected_command(system, test_id, "SystemTestSuite", processes=-1))

    @ddt.data('lms', 'cms')
    def test_SystemTestSuite_multi_processes(self, system):
        test_id = 'tests'
        suite = SystemTestSuite(system, test_id=test_id, processes=3)
        self.assertEqual(suite.cmd, self._expected_command(system, test_id, "SystemTestSuite", processes=3))
    ...
    def test_SystemTestSuite_with_xdist(self, system):
        ...
        self.assertEqual(
            suite.cmd,
            self._expected_command(system, test_id, "SystemTestSuite",
    ...
    def test_SystemTestSuite_with_xdist_multi_processes(self, system):
        ...
        self.assertEqual(
            suite.cmd,
            self._expected_command(system, test_id, "SystemTestSuite", processes=2,
                                   xdist_ip_addresses=XDIST_TESTING_IP_ADDRESS_LIST))
    ...
    def test_SystemTestSuite_with_xdist_negative_processes(self, system):
        ...
        self.assertEqual(
            suite.cmd,
            self._expected_command(system, test_id, "SystemTestSuite", processes=-1,
                                   xdist_ip_addresses=XDIST_TESTING_IP_ADDRESS_LIST))
    ...
    def test_LibTestSuite_suites(self, system):
        ...
        self.assertEqual(suite.cmd, self._expected_command(system, test_id, "LibTestSuite"))

    @ddt.data('common/lib/xmodule', 'pavelib/paver_tests')
    def test_LibTestSuite_auto_processes(self, system):
        test_id = 'tests'
        suite = LibTestSuite(system, test_id=test_id, processes=-1)
        self.assertEqual(suite.cmd, self._expected_command(system, test_id, "LibTestSuite", processes=-1))

    @ddt.data('common/lib/xmodule', 'pavelib/paver_tests')
    def test_LibTestSuite_multi_processes(self, system):
        test_id = 'tests'
        suite = LibTestSuite(system, test_id=test_id, processes=3)
        self.assertEqual(suite.cmd, self._expected_command(system, test_id, "LibTestSuite", processes=3))
    ...
    def test_LibTestSuite_with_xdist(self, system):
        ...
        self.assertEqual(
            suite.cmd,
            self._expected_command(system, test_id, "LibTestSuite",
                                   xdist_ip_addresses=XDIST_TESTING_IP_ADDRESS_LIST))
    ...
    def test_LibTestSuite_with_xdist_multi_processes(self, system):
        ...
        self.assertEqual(
            suite.cmd,
            self._expected_command(system, test_id, "LibTestSuite", processes=2,
                                   xdist_ip_addresses=XDIST_TESTING_IP_ADDRESS_LIST))
    ...
    def test_LibTestSuite_with_xdist_negative_processes(self, system):
        ...
        self.assertEqual(
            suite.cmd,
            self._expected_command(system, test_id, "LibTestSuite", processes=-1,
                                   xdist_ip_addresses=XDIST_TESTING_IP_ADDRESS_LIST))
